...
class GetRecipeResultsUseCase @Inject constructor(
    private val repository: RecipeRepository
) {
    operator fun invoke(query: String?): Flow<Resource<List<RecipeMetadata>>> = flow {
        try {
            emit(Resource.Loading())
            val recipes = repository.getRecipes(query).map { it.toRecipeMetadata() }
